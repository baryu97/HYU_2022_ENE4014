...
__global__ void maxpool(float *input, float *output, const int input_size, const int filter_size) {
    ...
    // CHANGE
    float filtered_max = input[(row * input_size + col)*filter_size];

    for(int x = row * filter_size; x < row * filter_size + filter_size; x ++  ){
        for(int y = col * filter_size; y < col * filter_size + filter_size; y ++ ){
            filtered_max = fmaxf(filtered_max,input[(x * input_size + y)]);
        }
    }
    output[row * output_size + col] = filtered_max;
}
...
//    if(row>=input_size ||col>=input_size) { return; }
    ...
    for(int p = 0; p < (input_size + TILE_WIDTH - 1)/TILE_WIDTH + 1; p++){
        // CHANGE
        ...
        if ((tx + p * TILE_WIDTH) < input_size && row < input_size)
//        if ((tx + p * TILE_WIDTH) < input_size)
            s_a[ty][tx] = a[row * input_size + p * TILE_WIDTH + tx];
        if ((ty + p * TILE_WIDTH) < input_size && col < input_size)
//        if ((ty + p * TILE_WIDTH) < input_size)
